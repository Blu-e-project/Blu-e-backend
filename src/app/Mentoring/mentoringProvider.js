...
exports.retrievePickMenteeCom = async function(pickId){
  const connection = await pool.getConnection(async (conn) => conn);
  const pickMenteeComListResult = await mentoringDao.selectMatchingCom(connection, pickId);
  connection.release();

  return pickMenteeComListResult
}

exports.retrievePickMentorCom = async function(pickId){
  const connection = await pool.getConnection(async (conn) => conn);
  const pickMentorComListResult = await mentoringDao.selectMatchingCom(connection, pickId);
  connection.release();
